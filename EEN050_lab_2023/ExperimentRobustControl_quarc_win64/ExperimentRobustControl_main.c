...
typedef RT_MODEL_ExperimentRobustCont_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * ExperimentRobustControl(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(ExperimentRobustControl_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(ExperimentRobustControl_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://ExperimentRobustControl:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: ExperimentRobustControl -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (ExperimentRobustControl_DW.HILReadEncoderTimebase_Task, (t_clock)
         ExperimentRobustControl_P.HILReadEncoderTimebase_clock, 1000.0, -1)
        ;
      if (result == 0) {
